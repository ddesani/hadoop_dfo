...
{
	char buffer[MAXLEN], *job_status, *job_time;
	double time;
	FILE *job_output_status;
	...
	/* Executar Job */
	// hadoop jar hadoop-examples-1.0.4.jar pi 200 5000000000
	snprintf(buffer, MAXLEN,
		"ssh hadoop@node-10  \"hadoop jar /mnt/hadoop-1.0.4/hadoop-examples-1.0.4.jar pi 200 5000000000\" | tail -n 2 >> /data/otimizacao/pi ");
	system(buffer);


	/* Verificar se execucao foi OK */
	job_output_status = popen("grep Estimated /data/otimizacao/pi | wc -l", "r");
	job_status = fgets(buffer, sizeof(buffer), job_output_status);
	if ( strcmp("0\n",job_status) == 0 )
		time = 10000;
	else{
		/* Salvar tempo */
		job_time = fgets(buffer, sizeof(buffer), popen("tail -n 2 /data/otimizacao/pi | head -n 1  | awk '{print $4}'", "r") );
	        printf("time : %s", job_time);
		time = atof(buffer);
	}
	return time;
}
...
double objetivo(unsigned n, double *x, double *grad, void *my_func_data)
{
	...
	for ( i = 0; i < REPETICOES; ++i ){
		...
		printf("PARCIAL.%d: %d, f = %g\n", i, evals, f);
		for (j = 0; j < N; ++j)
                	printf("PARCIAL.%d: %15g   %s\n", i, x[j], x_names[j]);
	}
	f = -f / REPETICOES;
